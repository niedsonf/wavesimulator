  ...
  Widget build(BuildContext context) {
    ...
    Timer timer = Timer.periodic(Duration(milliseconds: 100), (timer) {
      if (mounted) {
        ...
        setState(() {
          _angle += adjustController.velocity / 5000;
        });
      } else {
        _angle = 0;
      }
    ...
    return CustomPaint(
      ...
      size: Responsiveness.isLargeScreen(context) ? Size(ref * 0.75, ref * 0.75) :  Size(ref * 0.65, ref * 0.65),
    );
  }
}
...
class PlotterPaint extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double i = 0.1; i < 1; i += 0.1) {
      canvas.drawLine(Offset(0 + ref * i, 0), Offset(0 + ref * i, ref), paint);
      canvas.drawLine(Offset(0, ref * i), Offset(ref + 0, ref * i), paint);
    }
    ...
    paint
      ..strokeWidth = 1
      ..color = lightColor;
    canvas.drawLine(Offset(0, ref / 2), Offset(ref + 0, ref / 2), paint);
    canvas.drawRect(rect, paint);
  }
